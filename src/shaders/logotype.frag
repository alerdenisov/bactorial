...
void main() {
  float r = noise(vUv * 5. + time * 2.) * noise(vUv * 32. + time * .232);
  // gl_FragColor = vec4(r, 0.0, 0.0, 1.0);
  // return;
  float d = texture2D(map, vUv).x;
  d += r * 1.5 * d;
  float st = sin(time - vUv.x * 4.0) * 0.5 + 0.5;
  float e = 0. + st * 0.5;
  float s = smoothstep(e, 1.0, d);

  float b = pow(s, 1.5);//pow(saturate(d * 1.2), 5.0);
  b *= 4.0;
  b = floor(b);
  b /= 3.0;
  ...
  // vec3 dark = vec3(.168, 0.176, .243);
  vec3 dark = vec3(.078, 0.082, .113);
  // vec3 dark = vec3(1., 0.388, .501);
  vec3 light = vec3(.0, .0, .009);

  float mask = smoothstep(e, e + 0.01, s);
  vec3 color = mix(dark, light, b);
  color *= mask;

  gl_FragColor = vec4(color, mask);// * b;//pow(saturate(d * 1.1), 5.0);
  // gl_FragColor = vec4(mix(dark, light, b), 1.0);// * b;//pow(saturate(d * 1.1), 5.0);
}
// // varying vec2 vIndex;
